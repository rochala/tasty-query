...
/** **********************************************************
  ...
  * ************************************************************************************
  */
private[tasties] object TastyFormat:
  ...
  /** Tags used to serialize names.
    *
    * Should update [[TastyFormat$.nameTagToString]] if a new constant is added.
    */
  object NameTags:
    /** A simple name in UTF8 encoding. */
    final val UTF8 = 1

    /** A fully qualified name `<prefix>.<suffix>`. */
    final val QUALIFIED = 2

    /** An expanded name `<prefix>$$<suffix>`, used by Scala-2 for private names. */
    final val EXPANDED = 3

    /** An expansion prefix `<prefix>$<suffix>`, used by Scala-2 for private names. */
    final val EXPANDPREFIX = 4

    /** A unique name `<name>$<num>` where `<num>` is used only once for each `<name>`. */
    final val UNIQUE = 10

    /** The name `<meth-name>$default$<param-num>` of a default getter that returns a default argument. */
    final val DEFAULTGETTER = 11

    /** The name of a super accessor `super$name` created by SuperAccesors. */
    final val SUPERACCESSOR = 20

    /** The name of an inline accessor `inline$name`. */
    final val INLINEACCESSOR = 21

    /** The name of a synthetic method that retains the runtime body of an inline method. */
    final val BODYRETAINER = 22

    /** The name of an object class (or: module class) `<name>$`. */
    final val OBJECTCLASS = 23

    /** A pair of a name and a signature, used to identify possibly overloaded methods. */
    final val SIGNED = 63

    /** A triple of a name, a targetname and a signature.
      *
      * Used to identify possibly overloaded methods that carry an `@targetName` annotation.
      */
  ...
  def isModifierTag(tag: Int): Boolean = (tag: @switch) match
  ...
  def astTagToString(tag: Int): String = (tag: @switch) match
  ...
  /** If non-negative, the number of leading references (represented as nats) of a length/trees entry.
    ...
    */
  def numRefs(tag: Int): Int = (tag: @switch) match
    case VALDEF | DEFDEF | TYPEDEF | TYPEPARAM | PARAM | NAMEDARG | RETURN | BIND | SELFDEF | REFINEDtype | TERMREFin |
        TYPEREFin | SELECTin | HOLE =>
      1
